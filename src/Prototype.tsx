...
const activities = [
  { id: 1, name: "西岸美术馆", type: "展览", time: "10:00–12:00", cost: 80, image: "/assets/weekend/west-bund-museum.jpg", reason: "室内避暑 · 艺术偏好 94%", distance: "地铁 32 分钟", location: "上海 · 徐汇滨江", address: "龙腾大道 2600 号", description: "在黄浦江畔慢慢看一场展，把午后的光影和艺术都留给自己。学生证可享优惠，馆内动线舒适，也适合第一次约见的周末搭子。" },
  { id: 2, name: "龙华会周末市集", type: "市集", time: "12:30–14:00", cost: 58, image: "/assets/weekend/longhua-hui.jpg", reason: "午餐顺路 · 适合 3 人", distance: "步行 900 米", location: "上海 · 龙华会", address: "龙华路 2778 号", description: "从手作摊位逛到独立咖啡，午餐和小众好物一次解决。市集就在地铁上盖，雨天也能轻松抵达。" },
  { id: 3, name: "徐汇滨江日落散步", type: "城市漫步", time: "17:30–19:00", cost: 0, image: "/assets/weekend/xuhui-riverside.jpg", reason: "AI 已避开高温时段", distance: "骑行 1.8 公里", location: "上海 · 徐汇滨江", address: "龙腾大道滨江步道", description: "沿着江边从艺术区走到日落观景台，晚风、骑行和城市天际线都刚刚好。路线平缓，新手也能自在完成。" },
  { id: 5, name: "衡山路街区早午餐", type: "早午餐", time: "10:30–12:00", cost: 72, image: "/assets/weekend/longhua-hui.jpg", reason: "周日上午轻松出发", distance: "地铁 18 分钟", location: "上海 · 衡复风貌区", address: "衡山路 8 号", description: "从一顿松弛的早午餐开始周日，在梧桐街区里慢慢醒来，为接下来的城市漫步补充体力。" },
  { id: 6, name: "武康路建筑漫步", type: "城市漫步", time: "13:00–15:00", cost: 0, image: "/assets/weekend/xuhui-riverside.jpg", reason: "树荫路线 · 少走回头路", distance: "步行 2.4 公里", location: "上海 · 武康路", address: "武康路历史文化名街", description: "沿着武康路和安福路看老建筑、逛小店，路线以林荫路为主，适合边走边拍。" },
  { id: 7, name: "上海图书馆东馆", type: "文化", time: "15:30–17:30", cost: 0, image: "/assets/weekend/west-bund-museum.jpg", reason: "午后室内避暑", distance: "地铁 25 分钟", location: "上海 · 浦东新区", address: "合欢路 300 号", description: "把周末最后一站留给安静的阅读空间，在开阔明亮的建筑里休息，也为两天行程自然收尾。" },
];
...
const resolveStopImage = (stop: Stop) => activities.find((activity) => activity.id === stop.id)?.image || stop.image || "/assets/weekend/xuhui-riverside.jpg";
const repairBrokenImage = (event: React.SyntheticEvent<HTMLImageElement>) => {
  const image = event.currentTarget;
  if (image.dataset.fallbackApplied === "true") return;
  image.dataset.fallbackApplied = "true";
  image.src = "/assets/weekend/xuhui-riverside.jpg";
};
  ...
  const applyAi = (prompt = aiText) => {
    ...
    window.setTimeout(() => {
      ...
      if (normalized.includes("下雨") || normalized.includes("室内")) {
        setStops((current) => current.map((stop) => stop.id === 3 ? { ...stop, name: "西岸艺术中心雨天特展", time: "16:30–18:00", tag: "雨天室内", image: "/assets/weekend/west-bund-museum.jpg", cost: 60 } : stop));
        ...
        setStops((current) => current.some((stop) => stop.id === 4) ? current : [...current, { id: 4, name: "滨江咖啡快闪", time: "15:30–16:20", tag: "咖啡", image: "/assets/weekend/xuhui-riverside.jpg", cost: 32 }]);
...
function InteractiveItineraryScreen({ stops, setStops, expenses, memos, total, weatherAlert, setWeatherAlert, openModal, routeReady, onExplore, budget, people, aiNote, onBack, onOpenAi, onOpenStop }: ItineraryScreenProps) {
  ...
  return <section className="itinerary-screen video-plan-screen">
    <div className="map-pane">
      <img className="map-image" src="/assets/weekend/map-route.jpg" alt="上海徐汇周末路线地图" draggable="false" onError={repairBrokenImage} />
...
function BillHistoryScreen({ expenses, hotel, people, onOpenCurrent }: { expenses: Expense[]; hotel: string; people: number; onOpenCurrent: () => void }) {
  ...
  const trips = [
    { id: 1, date: "9月19日", title: "徐汇滨江周末计划", image: "/assets/weekend/xuhui-riverside.jpg", total: currentTotal, people, detail: `${expenses.length + (hotel ? 1 : 0)} 笔消费 · 待结算`, current: true },
    { id: 2, date: "8月29日", title: "西岸艺术一日漫游", image: "/assets/weekend/west-bund-museum.jpg", total: 168, people: 4, detail: "5 笔消费 · 已完成 AA" },
    { id: 3, date: "8月16日", title: "龙华会市集逛吃", image: "/assets/weekend/longhua-hui.jpg", total: 286, people: 3, detail: "7 笔消费 · 已完成 AA" },
  ];
...
function MemoHistoryScreen({ memos, onOpenCurrent }: { memos: Memo[]; onOpenCurrent: () => void }) {
  const [selectedTrip, setSelectedTrip] = useState<number | null>(null);
  const trips = [
    { id: 1, date: "9月19日", title: "徐汇滨江周末计划", image: "/assets/weekend/xuhui-riverside.jpg", count: memos.length, pending: memos.filter((memo) => !memo.done).length, preview: memos[0]?.text ?? "暂无备忘", current: true },
    { id: 2, date: "8月29日", title: "西岸艺术一日漫游", image: "/assets/weekend/west-bund-museum.jpg", count: 4, pending: 0, preview: "学生证、充电宝、预约二维码" },
    { id: 3, date: "8月16日", title: "龙华会市集逛吃", image: "/assets/weekend/longhua-hui.jpg", count: 3, pending: 0, preview: "雨伞、环保袋、空腹出发" },
  ];
