...
- The itinerary detail must keep Overview, Saturday, and Sunday as real interactive tabs with distinct rendered content; known stop images must resolve from canonical bundled assets so stale persisted URLs cannot break itinerary thumbnails.
...
- Explore tabs classify destination play types only. User-authored guides are the content of the large destination cards and must open the same guide text in the shared destination detail.
- Team cards reuse the image-led Explore card and the shared destination detail, with a dedicated team section for member, meeting-time, and meeting-place information.
- `我的同行` is a historical record screen. It must not route directly to the recruitment tab.
- Check-in is a quick personal moment record, while guide publishing is a structured public article flow; keep their fields, copy, and outcomes visibly distinct.
- Plan detail and AI conversation should follow the September 16 screen recording: map-led itinerary sheet and a separate full-screen AI chat with a fixed composer.
- Shared destination details always open on the overview tab, keep the back/save controls pinned above scrolling content, and keep the add-to-plan action visible in the viewport.
- In a plan, each place opens the shared destination detail; transport is shown as a note beneath the relevant place, while editing changes the place name, time, and cost instead of applying a preset delay.
- The plan AI command composer stays pinned at the bottom of the viewport and follows the simulated keyboard edge. AI shortcuts do not appear as ordinary itinerary utilities, and the map header links to AI conversation history.
- Plan creation from the plan list is a manual creation flow. Journal bills and memos are trip-indexed secondary pages, and recent journeys contain check-in records rather than published guides.
- A saved check-in is also a destination contribution: its text appears in that destination's reviews, and any attached check-in photo is added to the destination's on-site gallery.
